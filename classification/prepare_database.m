...
function db = prepare_database(src,feature_fun,opt)
	...
	if opt.parallel
		% parfor loop - note that the contents are the same as the serial
		% loop, but MATLAB doesn't seem to offer an easy way of deduplicating
		% the code.
		...
		parfor k = 1:length(src.files)
			...
			% Due to parallelization constraints, each file stores its feature
			% vectors in one index of the features cell array. The
			% corresponding object indices are stored in the obj_ind cell
			% array.
			features{k} = cell(1,length(file_objects));
			obj_ind{k} = file_objects;
			
			% Apply all the feature functions to the objects contained in x.
			% The output buf is a PxNxK vector, where P corresponds to number
			% of feature coeffi- cients, N corresponds to the number of
			% features - the number of "windows" - in each object, and K
	...
	% Determine the size of the features array. Each row corresponds to a
	% feature coefficient, each column corresponds to a feature vector.
	rows = size(features{nonempty(1)}{1},1);
	cols = sum(cellfun(@(x)(sum(cellfun(@(x)(size(x,2)),x))), ...
		features(nonempty)));
...
function out = apply_features(x,objects,feature_fun,opt)
	...
	for k = 1:length(feature_fun)
		if nargin(feature_fun{k}) == 2
			% If only two inputs are given, the feature function will handle
			% the cut ting up of x into objects itself.
			out{k,1} = feature_fun{k}(x,objects);
		elseif nargin(feature_fun{k}) == 1
			% If only one input is given, the feature function relies on us to
			% cut up the input x into objects and feed them to it.
	...
	if length(feature_fun) == 1
		...
		% Concatenate along the first dimension - the feature dimension - of
		% each element.
