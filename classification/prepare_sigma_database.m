% prepare_sigma_database_1d: Calculates the features from objects in a
% source.
% Usage
...
function db = prepare_sigma_database(src,feature_fun,opt)
if nargin < 3
    opt = struct();
end
...
%For each file in the db, we will take only a fourth of the total numberof  
%objects. We will take them randomly.

for k = 1:dfeats
    nb_objs=size(features{k}{2}.signal{1},3);
    permuted_objs_inds=rand_perm(nb_objs);
    nb_selected_objs=ceil(0.25*nb_objs);
    for f=1:length(features{k}{2}.signal)
        
        for j=1:nb_selected_objs
            feats{1}{f}=[feats{1}{f};single(features{k}{2}.signal{f}(:,:,permuted_objs_inds(j)))];
        end
    end
   for f=1:length(features{k}{2}.signal)

        for j=1:nb_selected_objs
            feats{2}{f}=[feats{2}{f};single(features{k}{2}.signal{f}(:,:,permuted_objs_inds(j)))];
        end
   end  
end
