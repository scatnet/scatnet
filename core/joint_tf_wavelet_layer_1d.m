...
%       coefficients for negative fr_j's, that is, chirplets going
%       downwards in log-frequency along time, as well as their mirrors.
...
%   wavelet_factory_1d twice, is which most of the computation is
%   concentrated.
% See also
...
for s = 1:2
    ...
    for p1 = 1:length(Z.signal)
        ...
        % (iii) if Z is Y_phi, we compute U_Phi. Otherwise, we append
        % Z_Phi at the end of the Z_Psi array.
        if s == 1
