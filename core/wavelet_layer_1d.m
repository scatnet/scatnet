...
function [U_phi, U_psi] = wavelet_layer_1d(U, filters, options)
	...
	U_phi.meta.scale = zeros(0,size(U.meta.scale,2));
	
	U_psi.signal = {};
	U_psi.meta.bandwidth = [];
	U_psi.meta.resolution = [];
	U_psi.meta.scale = zeros(0,size(U.meta.scale,2)+1);
	...
	for p1 = 1:length(U.signal)
		...
		U_phi.meta.scale(p1,:) = U.meta.scale(p1,:);
		...
		for p2 = find(~cellfun(@isempty,x_psi))
			...
			U_psi.meta.scale(r,:) = [U.meta.scale(p1,:) p2-1];
