...
% of the form  psi psi / phi psi / psi phi
...
filters_rotation = tiny_wavelets(2*options.L,3);%new !!!
...
%downsampler2 = @(nextj, lastres) (min(max( nextj - lastres - aa, 0),numel(filters.phi)-lastres-1));
...
next_bands = @(lastj)(lastj + 1:delta_j:J - 1);
  ...
  propagators.U{2} = @(x)wavelet_modulus_3d(x, filters, filters_rotation, ...
    ...
    next_bands, options );
% pooling :
% for rotation invariant texture descr :
