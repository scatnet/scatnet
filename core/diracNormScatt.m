function nScatt = normScatt(x,cascade)
...
Sdirac=scatt(dirac, cascade);

[Scatt order2]=reorderScatt(Sx);
   nSdirac=reorderScatt(Sdirac);
   
   lsig=2*N/2^(length(Sx{2}.signal));

  k=1;
    while k<2*N
          normd=norm(nSdirac(k:k+lsig-1));
          normx=norm(Scatt(1,k:k+lsig-1))/normd;
          normx = 10^(-6) * floor(normx * 10^(6));
          
          Scatt(k:k+lsig-1) = normx + order2(k)* 10^(-7);
          nSdirac(k:k+lsig-1)=normd^2;
    ...
    nScatt(round(p+1):round(p+step))=Scatt(k);
    p=p+step;
    end
