...
function [out,meta] = spec(in,filters,options)
	...
	out = frame_fm(1:end/2,:,:);

	resolution = round(log2(N1))-options.oversampling;

	meta.order = ones(1,size(out,1));
	meta.scale = [0:size(out,1)-1];
	meta.bandwidth = phi2_bw*ones(1,size(out,1));
	meta.resolution = resolution*ones(1,size(out,1));
