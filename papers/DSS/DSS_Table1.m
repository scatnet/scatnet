...
% need periodic boundary conditions because the symmetrization/desymmetrization
% does not preserve energy, especially at large T
opt.boundary = 'per';
M = 3;
...
for k = 1:length(files)
	...
	for l = 1:length(Js)
		...
		for m = 0:max(meta.order)
			t1 = t(meta.order==m,:);
			E{k}(l,m+1) = sum(abs(t1(:)).^2);
		end
	end
	fprintf('%.2f s\n',toc(tt));
end

E = reshape(cell2mat(E), [size(E{1}) length(E)]);
