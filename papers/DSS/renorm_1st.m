% RENORM_1ST Renormalize first-order coefficients by local L1 norm
%
% Usage
%    S = renorm_1st(S, x, Wop)
%
% Input
...
%       the absolute value of x averaged by Wop.

function [S, S1] = renorm_1st(S, x, Wop, epsilon, replace)
	if nargin < 4
		epsilon = 2^(-20); 
	end
	...
	S{2}.signal = cellfun(@(x)(x./(S1.signal{1}+epsilon*sub_multiplier)), S{2}.signal, ...
		'UniformOutput',0);
