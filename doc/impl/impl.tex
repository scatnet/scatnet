...
\mcode{S} is a cell array whose length \mcode{M} is the same as \mcode{Wop}. The integer \mcode{M} is equal to $\bar{m}+1$, where $\bar{m}$ is the maximal order of the scattering transform. Note that, in scientific papers, the order $m$ ranges from $0$ to $\bar{m}$ ; but since MATLAB is one-based, the corresponding layer index ranges from \mcode{1} to \mcode{M}, hence the need of an offset of \mcode{1} for the variable \mcode{m}.
...
After initializing \mcode{U\{1\}} to \mcode{x}, \mcode{scat} executes the following loop :

\begin{lstlisting}
for m = 0:M-1
   [S{1+m}, V] = Wop{m+1}(U{1+m});
