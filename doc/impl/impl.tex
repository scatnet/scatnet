...
of the functions \mcode{renorm_scat} and  \mcode{rdivide_scat} enables you to build a fully renormalized second order
...
Using the framework set in the code example of the function \mcode{rdivide_scat},  one could compute this feature using the lines below:
...
\begin{itemize}
    ...
    \item \mcode{train_opt.reweight}: reweight is $\in \{0,1\}$. If reweight is set to one, then for a 2-class SVM classication problem where the two classes are numbered $1$ and $2$, the function to minimize in the svm optimization problem will be :
    \begin{equation*}
        \operatorname*{arg\,min}_{w,b,\xi} \, \frac{1}{2}w^{T}w + \frac{C(N_1+N_2)}{N_1}\sum_{i=1}^{N_1}\xi_i +  \frac{C(N_1 + N_2)}{N_2}\sum_{i=1}^{N_2}\xi_i
    \end{equation*}
    
    where $N_1$ and $N_2$ are the total numbers of training features respectively for class $1$ and class $2$. This remains true for any N-class classification problem with N >= 2.
  \end{itemize}  
    ...
    In such case we should tell the svm that we want it to give the same importance to the individual error rate of each class and this is achieved by setting the training option \mcode{reweight} to 1. The function \mcode{classif_mean_err} is a derived function which computes the average recognition
error of a classification experiment.
    The following lines provide more information on how to use ScatNet to do svm classification.
