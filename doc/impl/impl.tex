...
The \mcode{wavelet_factory_2d} function takes a \mcode{size_in} vector of image size and optional parameters wrapped in an \mcode{options} structure and outputs a cell array of linear operators corresponding to successive wavelet transforms. Its signature  is given by
\begin{lstlisting}
[Wop, filters] = wavelet_factory_2d(size_in, options);
\end{lstlisting}
...
Since parameters can vary over a large domain, and a fine-grained parameter search can be quite costly, there is also an adaptive parameter search function, \mcode{svm_adaptive_param_search}. This function calls \mcode{svm_param_search}, recenters the \mcode{C}-\mcode{gamma} parameter grid on the optimal parameter combination while increasing the grid resolution,  then reapplies \mcode{svm_param_search}, and so on. This is done \mcode{train_opt.cv_depth} times, which by default equals $2$.
